        ...
        <div class="col-md-8">
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <form method="POST" action="{{ route('admin.user-role.update', $roleUser->idrole_user) }}">
                        @csrf
                        @method('PUT')
                            ...
                            <select name="iduser" class="form-control @error('iduser') is-invalid @enderror" required>
                                ...
                                @foreach ($users as $user)
                                    <option value="{{ $user->iduser }}" 
                                        {{ $roleUser->iduser == $user->iduser ? 'selected' : '' }}>
                                        {{ $user->nama }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            <select name="idrole" class="form-control @error('idrole') is-invalid @enderror" required>
                                ...
                                @foreach ($roles as $role)
                                    <option value="{{ $role->idrole }}"
                                        {{ $roleUser->idrole == $role->idrole ? 'selected' : '' }}>
                                        {{ $role->nama_role }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <select name="status" class="form-control @error('status') is-invalid @enderror" required>
                                ...
                                <option value="0" {{ $roleUser->status == 0 ? 'selected' : '' }}>Tidak Aktif</option>
                            </select>
                            @error('status')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
